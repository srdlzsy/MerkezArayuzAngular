...
export class FireDepoCikisFisleriCreateComponent extends DocsTaskDialogBase {
  ...
  private readonly safeCreateRetry = new SafeCreateRetryDraft<IFurpaCreateStockReceiptRequestApiDto>();
  ...
  protected readonly form = new FormGroup({
    ...
    description: new FormControl('Gun sonu zayiat', { nonNullable: true, validators: [Validators.maxLength(50)] }),
    adminWarehouseNo: new FormControl<number | null>(null),
    lines: new FormArray<ZayiatLineFormGroup>([])
  });

  protected get lines(): FormArray<ZayiatLineFormGroup> {
  ...
  protected searchStock(): void {
    ...
    if (query.length < 2) {
      this.stockError.set('Zayiat stogu aramak icin en az 2 karakter gir.');
      return;
    }
      ...
      .subscribe({
        next: (results: IFurpaProductSearchItemApiDto[]) => {
          ...
          if (!normalizedResults.length) {
            this.stockError.set('Aramana uygun zayiat stogu bulunamadi.');
          }
        },
  ...
  protected submit(): void {
    ...
    if (this.form.invalid) {
      ...
      this.submitError.set('Zayiat ust bilgilerini kontrol et.');
      return;
    }

    if (!this.lines.length) {
      this.submitError.set('Zayiat fisi icin en az bir kalem ekle.');
      return;
    }
    ...
    this.stokIslemleriService
      .createSubeIci('FireDepoCikisFisleri', this.buildRequest())
      .pipe(finalize(() => this.submitting.set(false)))
      .subscribe({
        next: (result: unknown) => this.close({ created: true, result }),
        error: (error: HttpErrorResponse) => {
          this.submitError.set(this.resolveErrorMessage(error, 'Zayiat fisi kaydedilemedi.'));
        }
      });
  }
  ...
  private createLineFormGroup(stock?: IFurpaProductSearchItemApiDto): ZayiatLineFormGroup {
